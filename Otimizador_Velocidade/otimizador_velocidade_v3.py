...
def resolver_coluna(col_config, col_padrao, opcional=False):
    ...
    # Suporte para múltiplas colunas separadas por vírgula (soma de máquinas)
    if ',' in str(col_config):
        cols = [c.strip() for c in str(col_config).split(',')]
        valid_cols = [c for c in cols if c in df.columns]
        if valid_cols: return ','.join(valid_cols)
...
def extrair_soma_colunas(df, col_str):
    if not col_str: return np.zeros(len(df))
    cols = [c.strip() for c in str(col_str).split(',')]
    soma = np.zeros(len(df))
    for c in cols:
        if c in df.columns:
            soma += df[c].fillna(0).values
    return soma

v_ech_real_hist = extrair_soma_colunas(df, COL_V_ECH)
...
def calcular_velocidade(b1, b2, b3, b4, velocidade_nominal={int(VELOCIDADE_NOMINAL_ECH)}, min_modulacao={v_red_final:.3f}, max_modulacao={MAX_MODULACAO}):
    ...
    v_final_cph = max(v_base, vel_reduzida)
    return round((v_final_cph / velocidade_nominal) * 100.0, 1)
"""
...
def gerar_codigo_controlador_grafana():
    import re
    # Valores padrão iniciais
    g_url = '"http://172.23.224.145:3000"'
    g_user = '"admin"'
    g_pass = '"!ambev2021"'
    g_token = '""'
    g_ds = '"13"'
    ...
    # Preservar o que já está preenchido pelo usuário no script (se existir)
    # Tenta ler primeiro do obter_dados_grafana.py, se não achar tenta do próprio controlador antigo
    arquivos_busca = ["obter_dados_grafana.py", "controlador_velocidade_grafana_v3.py"]
    for arquivo in arquivos_busca:
        if os.path.exists(arquivo):
            with open(arquivo, "r", encoding="utf-8") as f:
                conteudo = f.read()
                m = re.search(r'^GRAFANA_URL\s*=\s*(.+)$', conteudo, re.MULTILINE)
                if m: g_url = m.group(1).strip()
                m = re.search(r'^GRAFANA_USER\s*=\s*(.+)$', conteudo, re.MULTILINE)
                if m: g_user = m.group(1).strip()
                m = re.search(r'^GRAFANA_PASSWORD\s*=\s*(.+)$', conteudo, re.MULTILINE)
                if m: g_pass = m.group(1).strip()
                m = re.search(r'^GRAFANA_TOKEN\s*=\s*(.+)$', conteudo, re.MULTILINE)
                if m: g_token = m.group(1).strip()
                m = re.search(r'^DATASOURCE_SELECTOR\s*=\s*(.+)$', conteudo, re.MULTILINE)
                if m: g_ds = m.group(1).strip()
                m = re.search(r'^BUCKET\s*=\s*(.+)$', conteudo, re.MULTILINE)
                if m: g_bucket = m.group(1).strip()
                m = re.search(r'^MEASUREMENT\s*=\s*(.+)$', conteudo, re.MULTILINE)
                if m: g_meas = m.group(1).strip()
            break # Se achou no primeiro, não precisa olhar no segundo

    codigo_grafana = f"""#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
GRAFANA_TOKEN = {g_token}
DATASOURCE_SELECTOR = {g_ds}
BUCKET = {g_bucket}
MEASUREMENT = {g_meas}
...
class ControladorVelocidadeEnchedoraV3:
    def __init__(self, min_modulacao={v_red_final:.3f}, max_modulacao={MAX_MODULACAO}):
        self.min_modulacao = min_modulacao
        self.max_modulacao = max_modulacao
...
if __name__ == "__main__":
    main()
"""
    with open("controlador_velocidade_grafana_v3.py", "w", encoding="utf-8") as f:
        f.write(codigo_grafana.strip() + "\n")
    print(f"➔ Controlador Live via Grafana gerado com sucesso em 'controlador_velocidade_grafana_v3.py'.")

gerar_codigo_controlador_grafana()
