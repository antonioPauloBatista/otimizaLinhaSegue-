...
def simular_historico_com_regras_ia(dados_df, p, time_step, retornar_series=False):
    ...
    for i in range(len(dados_df)):
        ...
        if b2[i] <= 2.0 or b3[i] >= 99.0:
            ...
            # Máquina rodando: usa a VELOCIDADE REAL como base.
            # O otimizador só pode REDUZIR — nunca inflar acima do histórico.
...
def objetivo(x):
    ...
    score, _, _, _, _ = simular_historico_com_regras_ia(df, p, time_step_seconds)
...
print("="*65)
print("Pronto! Use esses parâmetros nas suas regras de controle do CLP.")
